...
void UShooterGameInstance::HandleDemoPlaybackFailure(EDemoFailure::Type FailureType, const FString& ErrorString, const bool bStoppedOrWillStop)
{
...
void UShooterGameInstance::RemoveExistingLocalPlayer(ULocalPlayer* ExistingPlayer)
{
	...
	{
		// Kill the player
		APawn* Pawn = ExistingPlayer->PlayerController->GetPawn();
		AShooterCharacter* MyPawn = Cast<AShooterCharacter>(Pawn);
		if ( MyPawn )
		{

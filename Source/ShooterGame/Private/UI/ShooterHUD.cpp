...
void AShooterHUD::DrawCrosshair()
{
	...
	if (PCOwner)
	{
		APawn* OwningPawn = PCOwner->GetPawn();
		AShooterCharacter* Pawn =  Cast<AShooterCharacter>(OwningPawn);
		if (Pawn && Pawn->GetWeapon() && !Pawn->IsRunning() && (Pawn->IsTargeting() || (!Pawn->IsTargeting() && !Pawn->GetWeapon()->bHideCrosshairWhileNotAiming)))
		{
...
void AShooterHUD::NotifyWeaponHit(float DamageTaken, struct FDamageEvent const& DamageEvent, class APawn* PawnInstigator)
{
	...
	APawn* Pawn = (PlayerOwner) ? PlayerOwner->GetPawn() : NULL;
	if (AShooterCharacter* Character = Cast<AShooterCharacter>(Pawn))
	{
		...
		if (DirIndex > -1)
		{
			const float DamageTakenPercentage = (DamageTaken / Character->Health);

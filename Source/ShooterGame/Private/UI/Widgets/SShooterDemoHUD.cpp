...
FText SShooterDemoHUD::GetPlaybackSpeed() const
{
	...
	if (PlayerOwner->GetWorldSettings()->GetPauser() == nullptr)
	{
...
{
	if (PlayerOwner.IsValid() && PlayerOwner->GetWorldSettings() != nullptr && PlayerOwner->GetWorldSettings()->GetPauser() != nullptr)
	{
...
void SShooterDemoHUD::OnPauseCheckStateChanged(ECheckBoxState CheckState) const
{
	...
	switch(CheckState)
	{
		...
		{
			WorldSettings->SetPauser(PlayerOwner->PlayerState);
			break;
		}

		case ECheckBoxState::Unchecked:
		{
			WorldSettings->SetPauser(nullptr);
			break;
		}

...
void FShooterIngameMenu::DetachGameMenu()
{
	...
	APlayerController* const PlayerController = PlayerOwner ? PlayerOwner->PlayerController : nullptr;
	if (AShooterPlayerController* const PCOwner = Cast<AShooterPlayerController>(PlayerController))
	{
...
void FShooterIngameMenu::ToggleGameMenu()
{
	...
	{
		// Hide the scoreboard
		if (AShooterPlayerController* PlayerController = Cast<AShooterPlayerController>(PCOwner))
		{
			AShooterHUD* const ShooterHUD = PlayerController->GetShooterHUD();
			if( ShooterHUD != NULL )
			{

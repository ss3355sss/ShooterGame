...
FString UShooterLocalPlayer::GetNickname() const
{
	...
	if(!bReentry)
	{
		...
		if(GameInstance)
		{
			...
			const TArray<ULocalPlayerEncryptedPtr>& LocalPlayers = GameInstance->GetLocalPlayers();
			for (int i = 0; i < LocalPlayers.Num(); ++i)
			{
				const ULocalPlayerEncryptedPtr LocalPlayerPtr = LocalPlayers[i];
				const ULocalPlayer* LocalPlayer = LocalPlayerPtr;
				if( this == LocalPlayer)
				{
					break;
				}
...
FString UShooterLocalPlayer::GetGameLoginOptions() const
{
	...
	FString Options;
	if (FParse::Value(FCommandLine::Get(), TEXT("-DebugLoginOptions="), Options))
	{
		return Options;
	}
#endif
